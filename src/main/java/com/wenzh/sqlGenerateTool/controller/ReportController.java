...
import com.wenzh.sqlGenerateTool.core.model.enums.ContentTypeEnum;
import com.wenzh.sqlGenerateTool.model.dto.ReportAddRequest;
import com.wenzh.sqlGenerateTool.model.dto.ReportQueryRequest;
import com.wenzh.sqlGenerateTool.model.dto.ReportUpdateRequest;
import com.wenzh.sqlGenerateTool.model.entity.*;
import com.wenzh.sqlGenerateTool.model.enums.ReportStatusEnum;
import com.wenzh.sqlGenerateTool.service.*;
...
public class ReportController {
    ...
    public BaseResponse<Long> addReport(@RequestBody ReportAddRequest reportAddRequest, HttpServletRequest request) {
        ...
        if (ContentTypeEnum.DIC.getValue().equals(type)) {
        ...
        else if (ContentTypeEnum.FIELD_INFO.getValue().equals(type)){
            FieldInfo fieldInfo = fieldInfoService.getById(report.getReportedId());
            if (fieldInfo == null) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "举报对象不存在");
            }
            report.setReportedUserId(fieldInfo.getUserId());
        }else if (ContentTypeEnum.TABLE_INFO.getValue().equals(type)){
            TableInfo tableInfo = tableInfoService.getById(report.getReportedId());
            if (tableInfo == null) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "举报对象不存在");
            }
            report.setReportedUserId(tableInfo.getUserId());
        }
